...
GITHASH=$(shell git log -1 | head -n 1 | cut -f 2 -d \   )
GITBRANCH=$(shell git branch --show-current)
GCCVERSION=$(shell $(GCC) --version | head -n 1)
BUILD_INFO=build_info
BUILD_INFO_HFILE=src/$(BUILD_INFO)_$(PROJNAME).h
BUILD_INFO_MODULE=$(BUILD_INFO)_$(PROJNAME)
...
COBS:=\
	$(foreach fname,$(LIBRARY_OBJECT_CSOURCEFILES) $(BUILD_INFO_MODULE) $(SWIG_OBJECTS),$(OUTOBS)/$(fname).o)
...
# ######################################################################
# The build-info header. We gather build information into statically
# declared structures so that the code can determine, at runtime, exactly
...
#ifndef H_BUILD_INFO_$(PROJNAME)
#define H_BUILD_INFO_$(PROJNAME)
#ifdef __cplusplus
extern "C" {
#endif

   const char *build_info_$(PROJNAME)_name (void);
   const char *build_info_$(PROJNAME)_version (void);
   const char *build_info_$(PROJNAME)_githash (void);
   const char *build_info_$(PROJNAME)_gitbranch (void);
   const char *build_info_$(PROJNAME)_target (void);
   const char *build_info_$(PROJNAME)_targetplatform (void);
   const char *build_info_$(PROJNAME)_hostname (void);
   const char *build_info_$(PROJNAME)_ccversion (void);
   const char *build_info_$(PROJNAME)_build (void);
   const char *build_info_$(PROJNAME)_timestamp (void);
   const char *build_info_$(PROJNAME)_homepage (void);
...
CFLAGS:=$(COMMONFLAGS_C) $(EXTRA_CFLAGS)
CXXFLAGS:=$(COMMONFLAGS_CXX) $(EXTRA_CXXFLAGS)
LD:=$(GCC)
...
all:	$(SWIG_WRAPPERS) real-all
	...
	@cp -R $(HEADERS) $(BUILD_INFO_HFILE) include
...
$(BUILD_INFO_HFILE) src/$(BUILD_INFO_MODULE).c:	build.config Makefile
	@printf '$(subst $(newline), \n,$(BUILD_INFO_H))' > $(BUILD_INFO_HFILE)
	@printf '$(subst $(newline), \n,$(BUILD_INFO_C))' > src/$(BUILD_INFO_MODULE).c

$(DEPS): $(HEADERS) $(BUILD_INFO_HFILE)
...
clean-release:
	@rm -rfv release wrappers $(BUILD_INFO_HFILE)

clean-debug:
	@rm -rfv debug wrappers $(BUILD_INFO_HFILE)
